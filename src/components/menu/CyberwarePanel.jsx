...
const CYBER_CATEGORIES = [
  { id: 'frontal', label: 'Frontal Cortex', side: 'left' },
  { id: 'arms', label: 'Arms', side: 'left' },
  { id: 'skeleton', label: 'Skeleton', side: 'left' },
  { id: 'nervous', label: 'Nervous System', side: 'left' },
  { id: 'integumentary', label: 'Integumentary System', side: 'left' },
  { id: 'operating', label: 'Operating System', side: 'right' },
  { id: 'face', label: 'Face', side: 'right' },
  { id: 'hands', label: 'Hands', side: 'right' },
  { id: 'circulatory', label: 'Circulatory System', side: 'right' },
  { id: 'legs', label: 'Legs', side: 'right' },
];

const createDefaultSheet = () => ({
  identificacao: { nome: '', handle: '' },
  cyberware: { equipados: {}, equipadosMigrated: false },
});
...
function uniqueByName(list) {
  const map = new Map();
  list.forEach((item) => {
    const key = normalizeText(item?.name || item?.nome || '');
    if (!key) return;
    map.set(key, { ...item, name: item.name || item.nome });
  });
...
function normalizeSheet(raw) {
  ...
  const cyber = { ...(base.cyberware || {}) };
  const equipados = { ...(cyber.equipados || {}) };
  const migrated = Boolean(cyber.equipadosMigrated);

  if (!migrated) {
    const nervous = [...(equipados.nervous || [])];
    if (cyber.neuralware?.linkNeural?.instalado) {
      nervous.push({
        name: 'Link Neural',
        slot: cyber.neuralware.linkNeural.slots || '',
      });
    }
    (cyber.neuralware?.opcoes || []).forEach((opt) => {
      if (!opt) return;
      const name = opt.nome || opt.name || String(opt);
      nervous.push({ name, slot: opt.slot ?? '' });
    });

    const face = [...(equipados.face || [])];
    (cyber.cyberopticos?.olhoEsquerdo?.opcoes || []).forEach((opt) => {
      if (!opt) return;
      face.push({ name: `Olho Esquerdo: ${opt}`, slot: '' });
    });
    (cyber.cyberopticos?.olhoDireito?.opcoes || []).forEach((opt) => {
      if (!opt) return;
      face.push({ name: `Olho Direito: ${opt}`, slot: '' });
    });

    if (nervous.length) equipados.nervous = uniqueByName(nervous);
    if (face.length) equipados.face = uniqueByName(face);
  }
  ...
  return {
    ...
    cyberware: {
      ...cyber,
      equipados,
      equipadosMigrated: migrated || Boolean(cyber.equipadosMigrated) || true,
    },
  };
}

export default function CyberwarePanel({ onBack, campaignId, playerId }) {
  const [sheet, setSheet] = useState(createDefaultSheet);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeStoredState({
      ...
      playerId,
      scope: SHEET_SCOPE,
      fallback: createDefaultSheet(),
      onChange: (data) => {
        setSheet(normalizeSheet(data));
        setHydrated(true);
      },
    });
    return unsubscribe;
  ...
  const equippedByCategory = useMemo(() => {
    const equipados = sheet.cyberware?.equipados || {};
    return CYBER_CATEGORIES.reduce((acc, cat) => {
      acc[cat.id] = Array.isArray(equipados[cat.id]) ? equipados[cat.id] : [];
      return acc;
    }, {});
  }, [sheet]);

  const selectedItems = equippedByCategory[activeCategory] || [];
  ...
  useEffect(() => {
    ...
    setActiveItem((prev) => prev && selectedItems.includes(prev) ? prev : selectedItems[0]);
  ...
  const handleAddCyberware = () => {
    ...
    playSound('button');
    setSheet((prev) => {
      const next = normalizeSheet(prev);
      const equipados = { ...(next.cyberware?.equipados || {}) };
      const list = Array.isArray(equipados[activeCategory]) ? [...equipados[activeCategory]] : [];
      list.push({ name, slot: newItemSlot.trim() });
      equipados[activeCategory] = list;
      return {
        ...next,
        cyberware: {
          ...(next.cyberware || {}),
  ...
  const handleBodySelect = (part) => {
    const nextCategory = BODY_PART_TO_CATEGORY[part] || activeCategory;
    playSound('button');
  ...
  const leftCategories = CYBER_CATEGORIES.filter((cat) => cat.side === 'left');
  const rightCategories = CYBER_CATEGORIES.filter((cat) => cat.side === 'right');
  const displayName = sheet.identificacao?.nome || 'Operador';

  return (
    <div className="menu-shell cyberware-shell">
      <div className="panel-header-row">
        <h2 className="menu-title">Cyberware</h2>
        <button className="menu-back" onClick={onBack}>VOLTAR</button>
      </div>

      <div className="cyberware-header">
        <div className="cyberware-title">
          {displayName}
          {sheet.identificacao?.handle ? (
            <span className="cyberware-handle">{sheet.identificacao.handle}</span>
          ) : null}
        </div>
        <div className="cyberware-subtitle">Mapa tatico de implantes equipados</div>
      </div>

      <div className="cyberware-layout">
        <div className="cyberware-column">
          {leftCategories.map((cat) => {
            const items = equippedByCategory[cat.id] || [];
            return (
              <button
                key={cat.id}
                type="button"
                className={`cyberware-slot-card ${activeCategory === cat.id ? 'is-active' : ''}`}
                onClick={() => handleSelectCategory(cat.id)}
              >
                <div className="cyberware-slot-title">{cat.label}</div>
                <div className="cyberware-slot-items">
                  {items.length ? items.slice(0, 3).map((item, idx) => (
                    <span key={`${cat.id}-${idx}`} className="cyberware-chip">
                      {item.name}
                    </span>
                  )) : (
                    <span className="cyberware-slot-empty">Vazio</span>
                  )}
                </div>
                <div className="cyberware-slot-count">{items.length} equipado</div>
              </button>
            );
          })}
        </div>

        <div className="cyberware-center">
          <div className="cyberware-body">
            <div className="cyberware-rings" />
            <div className="human-body">
              ...
              <svg
                ...
                onClick={() => handleBodySelect('shoulder')}
              >
              ...
              <svg
                ...
                onClick={() => handleBodySelect('cheast')}
              >
              ...
              <svg
                ...
                onClick={() => handleBodySelect('stomach')}
              >
              ...
              <svg
                ...
                onClick={() => handleBodySelect('hands')}
              >
            ...
            <div className="human-body-readout">
              Area: <span>{activeBodyPart || '---'}</span>
            </div>
          </div>
        </div>
      ...
      <div className="cyberware-details">
        <div className="cyberware-details-header">
          <div className="cyberware-details-title">
            {CYBER_CATEGORIES.find((cat) => cat.id === activeCategory)?.label || 'Sistema'}
          </div>
          <div className="cyberware-details-sub">Implantes instalados</div>
        </div>
        <div className="cyberware-details-grid">
          <div className="cyberware-details-list">
            {selectedItems.length ? (
              selectedItems.map((item, index) => (
                <button
                  key={`${item.name}-${index}`}
                  type="button"
                  className={`cyberware-item ${activeItem === item ? 'is-active' : ''}`}
                  onClick={() => handleSelectItem(item)}
                >
                  <span className="cyberware-item-name">{item.name}</span>
                  {item.slot ? <span className="cyberware-item-slot">Slot {item.slot}</span> : null}
                  <span
                    className="cyberware-item-remove"
                    onClick={(event) => {
                      event.stopPropagation();
                      handleRemoveCyberware(index);
                    }}
                  >
                    Remover
                  </span>
              ...
              <div className="cyberware-empty">Nenhum implante instalado.</div>
            )}
          </div>

          <div className="cyberware-details-panel">
            <div className="cyberware-details-card">
              <div className="cyberware-details-label">Detalhes</div>
              {activeItem ? (
                <>
                  <div className="cyberware-details-name">{activeItem.name}</div>
                  <div className="cyberware-details-meta">
                    {activeItem.slot ? `Slot ${activeItem.slot}` : 'Slot livre'}
                  </div>
                  <div className="cyberware-details-desc">
                    Implante registrado para o sistema {CYBER_CATEGORIES.find((cat) => cat.id === activeCategory)?.label}.
                  </div>
                </>
              ) : (
                <div className="cyberware-details-desc">Selecione um implante para ver detalhes.</div>
              )}
            </div>

            <div className="cyberware-add-card">
              <div className="cyberware-details-label">Adicionar cyberware</div>
              <input
              ...
              <input
                ...
                value={newItemSlot}
                onChange={(event) => setNewItemSlot(event.target.value)}
              />
              <button className="mission-add-btn" onClick={handleAddCyberware}>
                ADICIONAR
              </button>
            </div>
          </div>
